<template lang="pug">
  ...
  .SectionList(v-else)
    .SectionList__empty(:style="{ opacity: previewClosed ? 1 : 0 }")
...
<script>
  ...
  import { COLLECTION_PREVIEW_CLOSED } from '../../../../modules/_events';
  ...
  @Component({
    ...
    computed: mapGetters(['pageData', 'm', 't'])
  })
  export default class SectionList extends Vue {
    /**
     * Preview section closed state indicator
     *s
     * @type {Boolean}
     */
    previewClosed = false;
    ...
    mounted() {
      this.scrollTarget = this.$el.offsetTop - 150;

      bus.$on(COLLECTION_PREVIEW_CLOSED, this.setPreviewAndReveal);
    }

    destroyed() {
      bus.$off(COLLECTION_PREVIEW_CLOSED, this.setPreviewAndReveal);
    }

    setPreviewAndReveal() {
      this
        .revealEmpty()
        .setPreviewClosedState();
    }
     ...
     */
    revealEmpty() {
      TweenMax.fromTo('.SectionList__empty', 1, { opacity: 0 }, {
        opacity: 1,
        ease: Power1.easeOut
      });
    ...
    /**
     * Set image preview was closed (flag)
     *
     * @return {SectionList}
     */
    setPreviewClosedState() {
