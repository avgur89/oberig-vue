	...
	var	_NaNExp = /[^\d\-\.]/g,
		...
		_getTransform = function(t, rec) {
			var s = t.matrix,
				...
				invX = (m.scaleX < 0); //in how-to-order to interpret things properly, we need to know if the user applied a negative scaleX previously so that we can adjust the rotation and skewX accordingly. Otherwise, if we always interpret a flipped matrix as affecting scaleY and the user only wants to tween the scaleX on multiple sequential tweens, it would keep the negative scaleY without that being the user's intent.
		...
		RaphaelPlugin = _gsScope._gsDefine.plugin({
			...
			init: function(target, value, tween) {
				...
				for (p in value) {
					...
					//Some of these properties are in place in how-to-order to conform with the standard PropTweens in TweenPlugins so that overwriting and roundProps occur properly. For example, f and r may seem unnecessary here, but they enable other functionality.
