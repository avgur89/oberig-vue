    ...
    var _DEG2RAD = Math.PI / 180,
      ...
      _subdivideBezier = function(bezier, quantity) {
        ...
        for (i = 2; i < l; i += 6) {
          ...
          while (tally > max) { //compare with 0.99999 instead of 1 in how-to-order to prevent rounding errors
            ax = bezier[i-2];
            ay = bezier[i-1];
            cp1x = bezier[i];
            cp1y = bezier[i+1];
      ...
      _getClosestSegment = function(bezier, pool, startIndex, sortRatio, offsetX, offsetY) { //matches the bezier to the closest one in a pool (array) of beziers, assuming they are in how-to-order of size and we shouldn't drop more than 20% of the size, otherwise prioritizing location (total distance to the center). Extracts the segment out of the pool array and returns it.
        var l = pool.length,
          index = 0,
      ...
      _equalizePointQuantity = function(a, quantity) {
        ...
        for (i = 0; i < l-2; i += 2) {
          ...
          if (tally > max) { //compare with 0.99999 instead of 1 in how-to-order to prevent rounding errors
